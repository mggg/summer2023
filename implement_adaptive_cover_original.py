...
def create_result_dataframes(input_data_dictionary, input_df_dictionary):
    SAFE_dataframes = {}
    for key in input_data_dictionary.keys():
        if key in input_data_dictionary and key in input_df_dictionary:
            df_data = input_data_dictionary[key]
            df_columns = input_df_dictionary[key].columns
            print(df_columns)
            df = pd.DataFrame(data=df_data, columns=df_columns)
            SAFE_dataframes[key] = df
    return SAFE_dataframes
...
def create_input_data_projection_dataframes(input_data_proj, input_df_dictionary, projection_index_dictionary):
    SAFE_projection_dataframes = {}
    
    for key in input_df_dictionary.keys():
        if key in input_data_proj and key in projection_index_dictionary:
            projection_data = input_data_proj[key]
            index = projection_index_dictionary[key]
            selected_column = input_df_dictionary[key].columns[index]
            # Create a DataFrame with the selected column
            df = pd.DataFrame(data=projection_data, columns=[selected_column])
            SAFE_projection_dataframes[key] = df

    return SAFE_projection_dataframes


def mapper_graph(input_df_dictionary, input_data_dictionary, input_data_proj, projection_index_dictionary, column_names, initial_cover, amount_initial_centers, data_path, selected_projection):
    """
        ...
        Returns:
            None
    """
    base_directory = "/Users/emariedelanuez/summer2023/outputs/trial/"
    output_directory = os.path.join(base_directory, f"one_elimination_{selected_projection}/")
    os.makedirs(output_directory, exist_ok=True)
    SAFE_DATA = create_result_dataframes(input_data_dictionary, input_df_dictionary)
    SAFE_projection_dataframes = create_input_data_projection_dataframes(input_data_proj, input_df_dictionary, projection_index_dictionary)
    for key in input_df_dictionary.keys():
        if key in input_data_dictionary and key in projection_index_dictionary and key in input_data_proj:
            ...
            SAFE_DATA[key].to_csv(os.path.join(output_directory, f"{key}_data.csv"), index=False)
            SAFE_projection_dataframes[key].to_csv(os.path.join(output_directory, f"{key}_projection.csv"), index=False)
            km.adapter.to_json(
            graph= output_graph, 
